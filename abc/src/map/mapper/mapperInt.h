...
//    Map_NodeVec_t *     vFanouts;      // the array of fanouts of the gate
    ...
    long long int       numCutChoices; // the number of cut choices of the node exposed for mapping
    long long int*      cutChoices;    // the indices of cuts of the node exposed for mapping
}; 
...
struct Map_CutStruct_t_  
{
    ...
    long long int       cutIdx;        // the cut index of this cut w.r.t all the cuts
    unsigned            uCanon[2][2];  // the n-canonical forms of neg and pos phases respectively
};
...
struct Map_SuperStruct_t_  
{
    ...
    Map_Time_t          tDelaysF[6];   // the pin-to-pin delay constraints for the fall of the output
