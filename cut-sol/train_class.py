...
def main():
    ...
    for ep in range(num_epochs):
        ...
        for i, batch in enumerate(train_dl):
            ...
            class_raw_loss = loss_function(class_pred,batch_class_labels)
            weighted_loss = class_raw_loss * class_weights[:, batch_class_labels.long()].squeeze()
        ...
        if ep == 0:
            ...
            torch.save(delay_model.state_dict(), "model_weights/class_model/epoch-{}-val_loss-{:.4f}.pt".format(ep+1,optim_valid_loss))
        else:
    ...
    with open('valid_curve_class.pkl','wb') as f:
        pickle.dump(valid_curve,f)

    with open('train_curve_class.pkl','wb') as f:
        pickle.dump(train_curve,f)
        
    ##### Plotting ######    
    plotChart([i+1 for i in range(len(valid_curve))],valid_curve,"# Epochs","Loss","valid_loss","Class Validation Loss")
    plotChart([i+1 for i in range(len(train_curve))],train_curve,"# Epochs","Loss","train_loss","Class Training Loss")
