# PYTHONUNBUFFERED=1 nohup python train_qor.py > train_qor3_out.log 2> train_qor3_err.log &
...
def main():
    ...
    delay_model = DelayQoRPredictor(hidden1_dim=hidden1_dim).to(device)
    ...
    for ep in range(num_epochs):
        ...
        for i, batch in enumerate(train_dl):
            ...
            loss = loss_function(reg_pred,batch_reg_labels)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if ep == 0:
            ...
            torch.save(delay_model.state_dict(), "model_weights/qor_model/epoch-{}-val_loss-{:.4f}.pt".format(ep+1,optim_valid_loss))
        else:
    ...
    with open('valid_curve_qor3.pkl','wb') as f:
        pickle.dump(valid_curve,f)

    with open('train_curve_qor3.pkl','wb') as f:
        pickle.dump(train_curve,f)
        
    ##### Plotting ######    
    plotChart([i+1 for i in range(len(valid_curve))],valid_curve,"# Epochs","Loss","valid_loss","QoR 3 Validation Loss")
    plotChart([i+1 for i in range(len(train_curve))],train_curve,"# Epochs","Loss","train_loss","QoR 3 Training Loss")
